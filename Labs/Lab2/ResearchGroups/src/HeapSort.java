...
public static void buildMaxHeap(String[][] arr){
...
public static void maxHeapify(String[][] arr, int i, int heapSize){
        ...
        int leftVal = leftIdx != -1 ? arr[leftIdx].length : -1;
        int rightVal = rightIdx != -1 ? arr[rightIdx].length : -1;

        // max val is the parent for now
        int indexOfMaxVal =i;

        if (leftVal >arr[indexOfMaxVal].length) {
                indexOfMaxVal=leftIdx;
        }
        if (rightVal > arr[indexOfMaxVal].length) {
